 ...
 */
 @RunWith(classOf[JUnitRunner])
   ...
   */
  test("details why two plus one is not three") {
    assert(2 + 1 === 3)
  }
  ...
  test("Sum of an empty list of ints") {
    assert(sum(List()) === 0)  // maybe it should throw an exception?
  }
  
  test("Sum of a 1-element list is that element") {
    assert(sum(List(17)) === 17)
  }
  
  test("Sum of a 2-element list") {
  ...
  test("max of a 1-element list is that element") {
    assert(max(List(17)) === 17)
  }
  
  test("max of a 2-element list") {
    assert(max(List(-5, 5)) === 5)
  }

  test("max of a list with repeated elements") {
    assert(max(List(-5, -3, 0, -5, 1, 0)) === 1)
  }
}
